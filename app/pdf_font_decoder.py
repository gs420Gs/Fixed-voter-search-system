...
def _location(doc):
    ...
    result = {}
    patterns = {
        # FIX: real bug -- on this PDF's cover/header page, the label
        # "জেলা" (district) sometimes loses its vowel sign entirely during
        # font decoding, extracting as "জলা" instead. The original pattern
        # only matched the fully-correct spelling, so district silently
        # extracted as None for every record from this PDF. Made the "ে"
        # optional to tolerate this. Anchored to line-start (rather than
        # matching "জলা"/"জেলা" anywhere) so this does NOT also match the
        # "জলা" substring that appears inside "উপজেলা"/"উপেজলা" (upazila) --
        # verified this against real extracted text before shipping.
        "district": r"(?:^|\n)\s*জ(?:ে)?লা\s*[:：]?\s*([^\n]+)",
