...
def _run_document(document_id):
    ...
    try:
        ...
        for r in records:
            # FIX: real bug -- font_processing.py tags every record with an
            # "extraction_method" key (e.g. "native-font"), but VoterRecord
            # (the DB model) has no such column. The line below already
            # popped "ocr_used" for the same reason but missed this one, so
            # EVERY successful native-font extraction (the main code path
            # for real, non-scanned voter-list PDFs) crashed here with
            # "'extraction_method' is an invalid keyword argument for
            # VoterRecord" -- the document was marked "failed" even though
            # extraction itself had already succeeded, and zero records
            # were ever saved to the database. Verified against a real
            # 492-record voter list PDF: before this fix, 0/492 records
            # were persisted (silent data loss); after, all 492 save
            # correctly.
            r.pop("ocr_used", None)
            r.pop("extraction_method", None)
...
async def upload_pdfs(background_tasks: BackgroundTasks, files: list[UploadFile] = File(...), user=Depends(admin_user), db: Session = Depends(get_db)):
    ...
    for upload in files:
        ...
        # FIX: this used to also pass pdf_data=data, storing a *second*
        # full copy of every uploaded PDF as a BLOB inside the database
        # row (on top of the copy already written to disk at `target`).
        # For a handful of small PDFs that's harmless; at real voter-list
        # scale (tens of GB) it silently doubles storage needs and is the
        # single biggest reason a "free" hosted database fills up fast.
        # The file on disk (stored_path) is the source of truth now.
        doc = Document(filename=safe, stored_path=str(target), status="processing", progress_percent=0, current_page=0, total_pages=0, current_stage="queued", records_found=0, quality_status="unknown")
        db.add(doc)
        db.commit()
        db.refresh(doc)
